...
the thinking. No cloud service is involved, and nothing about the conversation
is stored anywhere but that Mac.

This is one half of
[esp32-chatgpt-voice-mode](https://github.com/thiernoyunus/esp32-chatgpt-voice-mode);
the Mac half is in `../mac`.
...
Start here: **[../SETUP.md](../SETUP.md)** walks through both halves end to
end, with a check after each step. The rest of this page assumes you have
already done that.
...
out exactly what was kept and what was removed. Their history is roughly a
thousand commits by about 150 people; this folder starts from one snapshot of
it, so that what came after is legible as the part that changed.

The `../mac` folder is not derived from xiaozhi — that half is original work.
